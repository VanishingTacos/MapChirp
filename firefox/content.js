// Firefox content script: injects a small page script to observe page fetches
// and listens for messages from that page. Core logic (fetching, DOM injection)
// is the same as the Chrome content script but without overriding page globals.
...
// Storage helpers: prefer `browser.storage.local` (Promise), fallback to `chrome.storage.local`
...
// Start loading cache immediately (before DOM is ready)
loadPersistentCache();
...
(function injectPageObserver() {
  try {
    ...
    script.src = (typeof browser !== 'undefined' && browser.runtime && browser.runtime.getURL)
      ? browser.runtime.getURL('page_inject.js')
      : chrome.runtime.getURL('page_inject.js');
...
window.addEventListener('message', (event) => {
  ...
  try {
    ...
    if (username && location) {
      ...
      if (sanitized) {
        locationCache.set(username.toLowerCase(), sanitized);
      }
    }
  } catch (e) {
    // ignore
...
async function fetchLocationData(username) {
  ...
  if (!persistentCacheLoaded) {
    try {
      const result = await storageGet([`loc_${normalizedUsername}`]);
